...
func TestSize_Negative1(t *testing.T) {
	a := Dimension{1, unit.Units(gss.Unit_Px)}
	b := Dimension{2, unit.Units(gss.Unit_Em)}
	_, err := a.Add(b)
	if err == nil {
...
func TestSize_Negative2(t *testing.T) {
	a := Dimension{1, unit.Units(gss.Unit_Px, gss.Unit_Px)}
	b := Dimension{2, unit.Units(gss.Unit_Px)}
	_, err := a.Add(b)
	if err == nil {
		t.Errorf("act: unexpected success")
	}
...
func TestMultiply(t *testing.T) {
	var (
		a        = Dimension{1, unit.Units(gss.Unit_Px)}
		b        = Dimension{2, unit.Units(gss.Unit_Em)}
		expected = Dimension{2, unit.Units(gss.Unit_Px, gss.Unit_Em)}
	)
	got, err := a.Mul(b)
	if err != nil {
		t.Errorf("act: %v", err)
...
func TestDivide_StripUnit(t *testing.T) {
	var (
		a        = Dimension{10, unit.Units(gss.Unit_Px)}
		b        = Dimension{2, unit.Units(gss.Unit_Px)}
		expected = Dimension{5, unit.Units()}
	)
	got, err := a.Div(b)
	if err != nil {
		t.Errorf("act: %v", err)
